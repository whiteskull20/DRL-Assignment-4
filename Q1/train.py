...
class ActorCritic(nn.Module):
    def __init__(self):
        ...
        self.actor_mean = nn.Linear(128, 1)  # Mean for Gaussian policy
        self.actor_log_std = nn.Linear(128, 1)  # Log standard deviation (learnable)
        self.critic = nn.Sequential(
            nn.Linear(3, 128),
            nn.ReLU(),
            nn.Linear(128, 128),
            nn.ReLU(),
            nn.Linear(128, 1)  # Single value function output
        )
    ...
    def forward(self, x):
        ...
        log_std = self.actor_log_std(a).clamp(-5, 2)  # Keep log_std in a reasonable range
        std = log_std.exp()  # Convert log_std to std
        value = self.critic(x)
        return mean, std, value

class Agent:
    def __init__(self, gamma=0.99, policy_noise=0.2):
    ...
    def select_action(self, state, deterministic=False):
        with torch.no_grad():
            ...
            action = action.clamp(-2, 2)  # Ensure action stays in valid range
    ...
    def train(self, batch):
        ...
        with torch.no_grad():
            ...
            advantage = torch.FloatTensor(advantage).unsqueeze(1)
        ...
        # Final PPO loss
        loss = p_loss + 0.5 * v_loss + 0.001 * entropy
        # Gradient step
...
def train_ppo(agent, env, epochs=500, steps_per_epoch=2048, update_epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            reward_buffer.append(reward/100)
        ...
        loss /= update_epochs
        print(f"Epoch {epoch + 1}/{epochs}, Average Reward: {np.mean(rewards[-50:]):.2f}, Std: {np.std(rewards[-50:]):.2f}, Loss: {loss}")
        if np.mean(rewards[-50:]) - np.std(rewards[-50:]) > -350:
            return
...
if __name__ == "__main__":
    ...
    env = make_env()
    agent = Agent()
    train_ppo(agent, env, epochs=10000, steps_per_epoch=2048, update_epochs=1)
    env.close()
    # Save the model
    agent.save("pendulum_model.pth")
